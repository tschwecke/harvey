...
module.exports = function() {
	...
	var verifySetupOrTeardownExistence = function(desiredSetupOrTeardownIds, actualSetupOrTeardownIds, errorPrefix) {
		...
		for(var i=0; i<desiredSetupOrTeardownIds.length; i++) {
			if(actualSetupOrTeardownIds.indexOf(desiredSetupOrTeardownIds[i]) === -1) {
				throw new Error(errorPrefix + 'The setup or teardown \'' + desiredSetupOrTeardownIds[i] + '\' doesn\'t exist');
			}
		}
	...
	var testSchema = T.Object({
		...
		"verifications": T.Array().includes(T.String().min(1)),
