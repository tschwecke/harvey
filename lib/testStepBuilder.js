...
module.exports = function() {
	...
	this.buildTestStep = function(testPhase, testStep, requestTemplates, responseTemplates, parameters, variables, timeout, status) {
		...
		if(testStep.repeat && testPhase !== 'test') {
			var testStepInvoker = buildRepeatedTests(testPhase, testStep, requestTemplates, responseTemplates, parameters, variables, timeout, status);
		}
		//Otherwise just add it once
		else {
			var testStepInvoker = createTestStepInvoker(testPhase, testStep, requestTemplates, responseTemplates, parameters, variables, timeout, status);
		}

		return testStepInvoker;
	};

	var buildRepeatedTests = function(testPhase, testStep, requestTemplates, responseTemplates, parameters, variables, timeout, status) {
		...
		for (var i = startIndex; i <= stopIndex; i++) {
			...
			var testStepInvoker = createTestStepInvoker(testPhase, testStep, requestTemplates, responseTemplates, parameters, testVariables, timeout, status);
	...
	var createTestStepInvoker = function(testPhase, testStep, requestTemplates, responseTemplates, parameters, variables, timeout, status) {
		var testStepInvoker = function(callback) {
			...
			try {
				...
				var req = constructRequest(testStep, requestTemplates, variables, timeout);
			}
			catch(e) {
			...
			if(shouldRun(testStep, variables)) {
				...
				request(req, function(error, response, body) {
					...
					//Attempt to convert the responses to json
					if(testStep._oauth && _.isString(response.body)) {
						try {
							response.body = JSON.stringify(response.body, null, 2);
						}
						catch(e){} //If an error occurred just leave the body as it is
					}
	...
	var constructRequest = function (testStep, requestTemplates, variables, timeout) {
		var retVal = null;
        ...
        var headerValue = util.parseValue(request.headers[header], variables);
        if (headerValue !== '') {
          headers[header] = headerValue;
			...
			var querystring = {};
			for(var qsVar in request.querystring) {
        var qsVal = util.parseValue(request.querystring[qsVar], variables);
        if (qsVal !== '') {
          querystring[qsVar] = qsVal;
        }
			}
      ...
      if(request.timeout !== undefined && request.timeout >= 0) {
        timeout = request.timeout;
			...
			retVal = {
				...
				uri: uri,
				headers: headers,
				json: !(testStep.request && testStep.request.oauth), //EXPERIMENTAL: If using oauth we should set the body as a string instead of json
				qs: querystring,
				timeout: timeout
			};
			...
			if(formPost) {
				...
				if(request.oauth) {
					try {
						retVal.body = JSON.stringify(retVal.body, null, 2);
					}
					catch(e){} //If an error occurred just leave the body as it is
				}
			}
	...
	var performActions = function(actions, testStep, variables) {
		...
		for(var j=0; j<actions.length; j++) {
			...
			for(var actionName in action) {
				...
				//Perform the action
				actionFn(actionInfo, variables, util.getParseValueFacade(variables, testStep), testStep);
			}
		}
	};
