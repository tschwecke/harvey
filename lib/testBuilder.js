...
module.exports = function() {
	...
	this.buildTest = function(test, setupAndTeardowns, requestTemplates, responseTemplates, variables) {
		...
		var testInvoker = function(callback) {
			...
			if(test.skip === true) {
				...
				for(var i=0; i<test.setup.length; i++) {
					...
					if(setup) {
						var setupInvoker = testStepBuilder.buildTestStep("testSetup", setup, requestTemplates, responseTemplates, test.variables);
						testSteps.push(setupInvoker);
					}
				}

				//Wire up the test
				var testInvoker = testStepBuilder.buildTestStep("test", test, requestTemplates, responseTemplates, test.variables);
				...
				for(var i=0; i<test.teardown.length; i++) {
					...
					if(teardown) {
						var teardownInvoker = testStepBuilder.buildTestStep("testTeardown", teardown, requestTemplates, responseTemplates, test.variables);
