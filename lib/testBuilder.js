		...
		var testInvoker = function(callback) {
			...
			if(test.skip === true) {
				...
				for(var i=0; i<test.verifications.length; i++) {
					var verification = util.findById(test.verifications[i], setupAndTeardowns);

					if(!verification) {
						callback(new Error('Unable to find the verification \'' + test.verifications[i]  + '\''));
						return;
					}

					var verificationInvoker = testStepBuilder.buildTestStep("testVerification", verification, requestTemplates, responseTemplates, testVariables, status);
