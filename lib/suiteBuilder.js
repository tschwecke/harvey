...
module.exports = function() {
	...
	this.buildSuite = function(suite, config, status) {
		...
		var suiteInvoker = function(callback) {
			...
			if(suite.suiteSetup) {
				for(var i=0; i<suite.suiteSetup.length; i++) {
					...
					var setupInvoker = testStepBuilder.buildTestStep('suiteSetup', setup, suite.requestTemplates, suite.responseTemplates, testStepInfo.parameters, variables, status);
			...
			if(suite.suiteTeardown) {
				for(var i=0; i<suite.suiteTeardown.length; i++) {
					...
					var teardownInvoker = testStepBuilder.buildTestStep('suiteTeardown', teardown, suite.requestTemplates, suite.responseTemplates, testStepInfo.parameters, variables, status);
