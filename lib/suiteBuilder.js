	...
	this.buildSuite = function(tests, suite, variables, status) {
		...
		var suiteInvoker = function(callback) {
			...
			async.series(suiteSteps, function(error, suiteStepResults) {
				//If any of the tests were repeating then we'll have an array of arrays of results.
				//Let's flatten that.
/*				for (var i = 0; i < suiteStepResults.length; i++) {
					if(_.isArray(suiteStepResults[i])) {
						suiteStepResults[i] = _.flatten(suiteStepResults[i], true);
					}
				};
*/
