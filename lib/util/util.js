...
module.exports = {
	...
	parseValue: function(value, variables, config, responseAsJson, testStep) {
		var parsedValue;

		if(_.isArray(value)) {
			parsedValue = [];
			for(var i=0; i<value.length; i++) {
				parsedValue.push(this.parseValue(value[i], variables, config, responseAsJson, testStep));
			}
		}
		else if(_.isObject(value)) {
			for(var prop in value) {
				if(prop === '$config') {
					parsedValue = config[value[prop]];
				}
				else if(prop.substr(0,1) === '$') {
					var action = actionFactory.getAction(prop, value[prop]);
					parsedValue = action.perform(responseAsJson, testStep, variables, config);
				}
			}
			...
			if(!parsedValue) {
				parsedValue = {};
				for(var prop in value) {
					parsedValue[prop] = this.parseValue(value[prop], variables, config, responseAsJson, testStep);
				}
			}
		}
	...
	extractWithDotNotation: function (object, dotNotation) {
