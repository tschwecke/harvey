var _ = require('underscore'),
	...
	operatorFactory = require('../operators/operatorFactory.js');
		...
		if(_.isArray(expectedValue)) {
			...
			if(containsComparisonFunction) {
				...
				for(var propName in expectedValue) {
					...
					var operator = operatorFactory.getOperatorFunction(propName);

					if(operator) {
						var match = operator(actualValue, comparisonValue);
						if (!match) {
						...
						//If we couldn't find an operator by that name, check for an extractor
